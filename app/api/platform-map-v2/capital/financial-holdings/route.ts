...
import financialConfig from "../../../../../data/platform-map-v2/dart-financial.json";
import { createCacheStore } from "../../../../../lib/platform-map-v2/cache";
import { extractHoldingsFromText } from "../../../../../lib/platform-map-v2/capital/extract";
import { fetchDartReportText, fetchDartReports, loadCorpCodeMap, resolveCorpCode } from "../../../../../lib/platform-map-v2/capital/dart";
...
const CACHE_KEY = "pmv2:capital:auto:financial";
...
export async function GET(request: NextRequest) {
  ...
  for (const item of financialConfig.entities) {
    ...
    let reports;
    try {
      reports = await fetchDartReports(apiKey, corpCode, financialConfig.reportTypes);
    } catch (error) {
  ...
  console.info(LOG_PREFIX, "financial holdings sync", { count: entities.length, warnings: warnings.length });
  return response;
}
