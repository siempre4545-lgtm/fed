  ...
  try {
    ...
    if (readingAnalysis) {
      if (readingAnalysis.includes("증가 속도 둔화")) {
        // 1. 증가 속도 둔화 (1차 경고)
        overallComment = `📊 판독 결과: **증가 속도 둔화 (1차 경고)**
        
은행 준비금은 증가하고 있지만, 증가 속도가 이전 기간 대비 크게 둔화되었습니다. 이는 '📊 판독' 항목에서 설명한 1차 경고 신호입니다.

**의미:** 은행 간 신뢰의 척도가 변화하고 멈추기 시작하는 신호입니다. 돈의 양이 아니라 은행들이 서로를 얼마나 신뢰하는지, 그리고 그 신뢰가 어떻게 변화하는지를 관찰하는 지표에서 중요한 전환점입니다.

**경제 코치 해설:** 은행들이 서로를 신뢰하면서도 그 신뢰의 속도가 느려지고 있다는 것은, 은행 간 신뢰가 약화되기 시작하는 신호일 수 있습니다. 거대 자본가들은 이런 시점에 신중하게 관찰하며, 증가 속도가 계속 둔화되는지 확인합니다.`;
      } else if (readingAnalysis.includes("거의 수평")) {
        // 2. 거의 수평 (뉴스가 아무 말도 안하는 시기)
        overallComment = `📊 판독 결과: **거의 수평 (뉴스가 아무 말도 안하는 시기)**
        
은행 준비금이 거의 변동하지 않고 수평선에 가까운 상태입니다. 이는 '📊 판독' 항목에서 설명한 "뉴스가 아무 말도 안하는 시기"입니다.

**의미:** 은행들이 서로를 신뢰하지 않으면서도 위기라고 판단하지 않는 상태입니다. 이는 뉴스가 아무 말도 하지 않는 시기로, 조용한 변화가 일어나고 있음을 의미합니다.

**경제 코치 해설:** 조용한 변화가 일어나는 시기입니다. 은행 신뢰의 척도가 멈춘 상태로, 이는 단순한 정체가 아니라 은행 간 신뢰의 근본적인 변화가 진행 중일 수 있습니다. 거대 자본가들은 이런 시점을 매우 주의 깊게 관찰하며, 다음 단계(하락 전환 또는 정상 복귀)를 예의주시합니다.`;
      } else if (readingAnalysis.includes("하락 속도 가속")) {
        // 4. 하락 속도 가속 (경고. 언제 터질까의 문제)
        overallComment = `📊 판독 결과: **하락 속도 가속 (경고. 언제 터질까의 문제)**
        
은행 간 신뢰가 후퇴하고 있으며, 하락 속도가 빨라지고 있습니다. 이는 '📊 판독' 항목에서 설명한 가장 심각한 경고 신호입니다.

**의미:** 하락이 시작된 후 속도가 빨라지면, 은행 간 신뢰가 급격히 후퇴하고 있다는 의미입니다. 이는 언제 터질까의 문제로, 위기 전조 신호입니다.

**경제 코치 해설:** 은행들이 서로를 신뢰하지 않고 중앙은행으로 돌아가기 시작했으며, 그 속도가 가속화되고 있습니다. 은행 신뢰의 척도가 급격히 변화하고 멈추는 시점입니다. 거대 자본가들은 이런 시점에 방어적 포지션으로 전환하며, 리스크가 큰 우선순서대로 자산을 정리합니다. 공격적 투자는 중단하고, 현금 확보로 선택지를 넓히는 시기입니다.`;
      } else if (readingAnalysis.includes("하락 전환")) {
        // 3. 하락 전환 (은행 간 신뢰 후퇴)
        overallComment = `📊 판독 결과: **하락 전환 (은행 간 신뢰 후퇴)**
        
은행 간 신뢰가 후퇴하기 시작했습니다. 이는 '📊 판독' 항목에서 설명한 "은행 간 신뢰 후퇴" 단계입니다.

**의미:** 준비금이 감소하기 시작하면, 은행들이 서로를 신뢰하지 않고 중앙은행으로 돌아가기 시작했다는 신호입니다.

**경제 코치 해설:** 은행 신뢰의 척도가 변화하고 멈추는 중요한 전환점입니다. 은행들이 서로를 포기하고 중앙은행을 선택지로 고려하기 시작했다는 의미입니다. 거대 자본가들은 이런 시점에 리스크 관리에 집중하며, 지속적인 관찰을 통해 하락 속도가 빨라지는지, 아니면 정상으로 복귀하는지 확인합니다.`;
      } else if (readingAnalysis.includes("정상 증가")) {
        // 정상 증가
        overallComment = `📊 판독 결과: **정상 증가**
        
은행 준비금이 안정적으로 증가하고 있습니다. 은행 간 신뢰가 유지되고 있는 상태입니다.

**의미:** 은행들이 서로를 신뢰하고 자금을 순환시키고 있다는 긍정적인 신호입니다. 은행 신뢰의 척도가 정상적으로 작동하고 있습니다.

**경제 코치 해설:** 유동성 환경이 개선되고 있으며, 신용 창출이 활발해질 수 있는 환경입니다. 거대 자본가들은 이런 시점에 성장 자산의 비중을 늘립니다. 다만, 증가 속도가 둔화되는지 계속 관찰해야 합니다.`;
      }
