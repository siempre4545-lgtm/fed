...
const buildReason = (
  ...
  if (status === "정합") {
    return hasHoldings
      ? `실제 보유(${holdingsCount}건)와 점수 신호가 동시에 관측됩니다.`
      : "점수와 자본 이동 지표가 같은 방향으로 움직입니다.";
  }
  if (status === "선행") {
    return hasHoldings
...
const buildSummary = (
  ...
  if (status === "정합") {
    return `자본 이동 지표(${alignmentBand})와 실제 보유(${holdingsCount}건)가 일치합니다.`;
  }
  if (status === "선행") return `점수 신호가 자본 이동 지표(${alignmentBand})보다 앞서 있습니다.`;
  if (status === "후행") return `자본 이동 지표(${alignmentBand}) 대비 점수 반영이 느립니다.`;
  return `점수 신호 대비 자본 이동 지표(${alignmentBand})는 약합니다.`;
};
...
export const buildCapitalComparison = (params: {
  ...
  if (!hasHoldings && rating.totalScore >= SCORE_VERY_HIGH && alignmentScore <= ALIGN_LOW) {
    ...
    status = alignmentScore >= ALIGN_HIGH ? "정합" : "후행";
  }
  ...
  return {
    ...
    reason: buildReason(status, hasHoldings, holdingsCount),
    summary: buildSummary(status, alignmentBand, holdingsCount),
    holdings,
    institutionTypes,
  };
};
